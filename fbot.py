...
MAX_DRAW_DOWN_LIMIT = 0.01  # Stop trading if drawdown exceeds 1%
MAX_STOCK_ALLOCATION = 0.20  # Maximum 20% of portfolio in one stock
TRAILING_STOP_PERCENT = 0.125  #  Trailing stop of .125 %
...
def calculate_position_size(ticker, portfolio_value, max_risk_per_trade=MAX_RISK_PER_TRADE):
    ...
    if var != 0:
        ...
        available_shares = (buying_power//3) // stock_price
        ...
        #final_position_size = round(position_size) * 10
        final_position_size = round(available_shares)
...
def set_trailing_stop(ticker, position_size):
    try:
        # Pause briefly before checking the latest order
        time.sleep(3)
        ...
        if position.side == 'long':
            print(f"Setting trailing stop for long position on {ticker} with {position_size} shares.")
            
            # Define the stop loss parameters for a long position
            api.submit_order(
                symbol=ticker,
                qty=position_size,
                side='sell',  # Sell to exit a long position
                type='trailing_stop',
                time_in_force='gtc',  # Good-till-cancelled
                trail_percent=TRAILING_STOP_PERCENT  # The trailing stop percent defined in your code
            )
        elif position.side == 'short':
            print(f"Setting trailing stop for short position on {ticker} with {position_size} shares.")
            
            # Define the stop loss parameters for a short position
            api.submit_order(
                symbol=ticker,
                qty=position_size,
                side='buy',  # Buy to cover a short position
                type='trailing_stop',
                time_in_force='gtc',  # Good-till-cancelled
                trail_percent=TRAILING_STOP_PERCENT  # The trailing stop percent defined in your code
            )
        else:
            print(f"No valid position (long or short) found for {ticker}.")
    except Exception as e:
...
def subscribe_to_tickers():
    ...
    print(top_stock_today)
    #sp500_tickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'GME', 'AMC']  # Example tickers
    for ticker in top_stock_today:
...
async def check_time_for_liquidation():
    ...
    if current_time >= liquidation_time:
        ...
        await asyncio.sleep(600)  # 10-minute wait using asyncio
...
# Function to run the liquidation check every minute
async def run_liquidation_scheduler():
    while True:
        await check_time_for_liquidation()
        await asyncio.sleep(60)  # Check every 60 seconds

# Add this to your main asyncio event loop in the bot
async def run_trading_bot():
    logging.info("Starting trading bot...")
    if check_portfolio() and check_drawdown():
        subscribe_to_tickers()
        asyncio.create_task(run_liquidation_scheduler())  # Start the liquidation scheduler asynchronously
        await start_stream()  # Start streaming prices and trading
"""
